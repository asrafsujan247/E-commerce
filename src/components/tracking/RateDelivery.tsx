...
interface RateDeliveryBoyPayload {
  orderId: string;
  rating: number;
  review: string;
}

interface RateDeliveryBoyResult {
  success: boolean;
  error?: string;
}

async function rateDeliveryBoy(
  _payload: RateDeliveryBoyPayload,
  _token?: string,
): Promise<RateDeliveryBoyResult> {
  return { success: true };
}

const RateDelivery: React.FC<RateDeliveryProps> = ({
  orderId,
  existingRating,
}) => {
  const { user } = useAuth();
  ...
  const [submitted, setSubmitted] = useState<boolean>(!!existingRating?.rating);
