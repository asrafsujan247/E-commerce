...
interface SearchScreenElectronicProps {
  products?: Product[];
...
const SearchScreenElectronic: React.FC<SearchScreenElectronicProps> = ({
  products,
  attributes,
  categories,
  searchQuery,
  ...
  const toggleSection = useCallback(
    (s: string) => setExpandedSections((p) => ({ ...p, [s]: !p[s] })),
    [],
  );

  type SortableProduct = Product & { prices?: { price?: number }; createdAt?: string; reviews?: unknown[] | number };

  const sortedProducts = useMemo(() => {
    if (!productData) return [];
    const sorted = [...productData] as unknown as SortableProduct[];
    switch (sortBy) {
      case "price-low":
      ...
      case "popular":
        sorted.sort(
          (a, b) => (Array.isArray(b.reviews) ? b.reviews.length : 0) - (Array.isArray(a.reviews) ? a.reviews.length : 0),
        );
        break;
      default:
        break;
  ...
  const handleCategoryClick = (id: string) => navigate(`/search?_id=${id}`);
