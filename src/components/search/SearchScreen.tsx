...
const FilterOption = ({
  ...
  >
    <span
      className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center transition-colors ${active ? "border-primary" : "border-border"}`}
    >
...
const ProductSearchScreen = ({
  ...
  // Sync slider/URL category selection → sidebar filter state
  useEffect(() => {
    if (!selectedCategory) {
      setSelectedDept("");
  ...
  const availableSubCats = useMemo<SubCat[]>(() => {
    ...
    return (
      deptData
        ...
        .find((c) => c._id === selectedFilterCat)?.children ?? []
    );
  }, [selectedFilterCat]);
  ...
  const allSuppliers = useMemo(
    () =>
      [
        ...new Set((products as any[]).map((p) => p.supplier).filter(Boolean)),
      ].sort() as string[],
    [products],
  );
    ...
    if (selectedSubCat) {
      ...
      result = result.filter((p) =>
        (p as any).categories?.some(
          (c: any) => c._id === selectedFilterCat || subIds.includes(c._id),
        ),
      );
    } else if (selectedDept) {
      const dept = deptData.find((d) => d._id === selectedDept);
      if (dept) {
        const allIds = dept.categories.flatMap((c) => [
          c._id,
          ...c.children.map((s) => s._id),
        ]);
        result = result.filter((p) =>
  ...
  useEffect(() => {
    ...
    if (!sentinel || visibleProduct >= filteredProducts.length) return;
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) setVisibleProduct((p) => p + 24);
      },
      { rootMargin: "100px" },
    );
  ...
  const activeFilterCount =
    ...
    (priceMin > 0 || priceMax < globalPriceMax ? 1 : 0);
  ...
  const filterContent = (
    <div className="p-4">
      ...
      <FilterSection
        title="Department"
        isOpen={expanded.department}
        onToggle={() => toggleExpanded("department")}
      >
        <FilterOption
      ...
      {availableSubCats.length > 0 && (
        <FilterSection
          title="Subcategory"
          isOpen={expanded.subcategory}
          onToggle={() => toggleExpanded("subcategory")}
        >
          <FilterOption
      ...
      {allSuppliers.length > 0 && (
        <FilterSection
          title="Supplier"
          isOpen={expanded.supplier}
          onToggle={() => toggleExpanded("supplier")}
        >
          <FilterOption
            label="All Suppliers"
        ...
        <div className="flex gap-6">
          ...
          {showFilters && (
            <aside className="hidden md:block w-64 shrink-0">
              <div className="bg-white rounded-sm sticky top-20 max-h-[calc(100vh-6rem)] overflow-y-auto">
                {filterContent}
              </div>
            </aside>
          )}
