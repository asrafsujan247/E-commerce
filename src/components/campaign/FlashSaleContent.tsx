...
import { ProductAttribute, TranslationMap } from "@appTypes/index";

interface CampaignProductEntry {
  product?: { _id?: string; [key: string]: unknown };
...
type TabKey = "all" | "active" | "upcoming";
...
const FlashSaleContent: React.FC<FlashSaleContentProps> = ({
  campaigns,
  attributes,
}) => {
  ...
  const activeCampaigns = campaigns.filter(
    (c) => c.startTime && c.endTime && new Date(c.startTime) <= now && new Date(c.endTime) >= now,
  );
      ...
      {displayCampaigns.length > 0 ? (
        <div className="space-y-10">
          {displayCampaigns.map((campaign) => {
            const isActive = !!(campaign.startTime && campaign.endTime &&
              new Date(campaign.startTime) <= now &&
              new Date(campaign.endTime) >= now);
            const isUpcoming = !!(campaign.startTime && new Date(campaign.startTime) > now);
